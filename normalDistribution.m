function varargout = normalDistribution(varargin)
% NORMALDISTRIBUTION MATLAB code for normalDistribution.fig
%      NORMALDISTRIBUTION, by itself, creates a new NORMALDISTRIBUTION or raises the existing
%      singleton*.
%
%      H = NORMALDISTRIBUTION returns the handle to a new NORMALDISTRIBUTION or the handle to
%      the existing singleton*.
%
%      NORMALDISTRIBUTION('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in NORMALDISTRIBUTION.M with the given input arguments.
%
%      NORMALDISTRIBUTION('Property','Value',...) creates a new NORMALDISTRIBUTION or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before normalDistribution_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to normalDistribution_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help normalDistribution

% Last Modified by GUIDE v2.5 15-Oct-2015 20:27:22

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @normalDistribution_OpeningFcn, ...
                   'gui_OutputFcn',  @normalDistribution_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before normalDistribution is made visible.
function normalDistribution_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to normalDistribution (see VARARGIN)

% Choose default command line output for normalDistribution
...
% UIWAIT makes normalDistribution wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = normalDistribution_OutputFcn(hObject, eventdata, handles) 
...
x2 = getappdata(0, 'x2');
y1 = x1:.1:x2;
norm1 = normpdf(y1, 0, 1);
axes(handles.axes1);
plot(y1, norm1);
setappdata(0, 'normalDistribution1', norm1);
end
...
x4 = getappdata(0, 'x4');
y2 = x3:.1:x4;
norm2 = normpdf(y2, 0, 1);
axes(handles.axes2);
plot(y2, norm2);
setappdata(0, 'normalDistribution2', norm2);
end
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
x1 = getappdata(0, 'normalDistribution1');
x2 = getappdata(0, 'normalDistribution2');

%Input Error handling
if ~isvector(x1) || ~isvector(x2)
   set(handles.edit5, 'String', sprintf('MWWTEST requires vector rather than matrix data.'));
end 
if ~all(isfinite(x1)) || ~all(isnumeric(x1)) || ~all(isfinite(x2)) || ~all(isnumeric(x2))
    set(handles.edit5, 'String', sprintf('Warning: all X1 and X2 values must be numeric and finite'));
end
...
% --- Executes during object creation, after setting all properties.
tail = str2double(get(handles.edit8, 'String'));
...
alpha = 0.5;
if ~isvector(x1) || ~isvector(x2)
   set(handles.edit6, 'String', sprintf('TESTT requires vector rather than matrix data.'));
end 
if ~all(isfinite(x1)) || ~all(isnumeric(x1)) || ~all(isfinite(x2)) || ~all(isnumeric(x2))
    set(handles.edit6, 'String', sprintf('Warning: all X1 and X2 values must be numeric and finite'));
end
...
switch tst
    case 0 
        ...
        set(handles.edit6, 'String', sprintf('STUDENT''S T-TEST FOR UNPAIRED SAMPLES\nt-test: %0.5f\nDF: %0.4f\ntail: %d\np-value: %0.4f\nhypothesis: %i\nPower is: %0.4f\n%s', t, gl, tail, p, h, x, str));
    case 1 
        n=length(x1); 
        gl=n-1; 
        d=x1-x2; 
